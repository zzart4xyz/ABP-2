...
def build_more_page(app):
    ...
    input_frame = QFrame()
    input_frame.setObjectName('remInputCard')
    input_frame.setStyleSheet(
        f"QFrame#remInputCard {{ background:{CLR_PANEL}; border-radius:18px; border:1px solid rgba(255,255,255,0.04); }}"
    )
    ih = QVBoxLayout(input_frame)
    ih.setContentsMargins(16, 16, 16, 16)
    ih.setSpacing(14)
    form_head = QHBoxLayout()
    form_title = QLabel('Crear nuevo recordatorio')
    form_title.setStyleSheet(f"color:{CLR_TITLE}; font:600 18px '{FONT_FAM}';")
    form_head.addWidget(form_title)
    form_head.addStretch(1)
    ih.addLayout(form_head)
    form_desc = QLabel('Define un mensaje claro y programa la fecha para recibir la alerta justo a tiempo.')
    form_desc.setStyleSheet(f"color:{CLR_TEXT_IDLE}; font:500 13px '{FONT_FAM}';")
    form_desc.setWordWrap(True)
    ih.addWidget(form_desc)
    ...
    btn_add_rec = QPushButton(' Añadir recordatorio')
    btn_add_rec.setIcon(icon('Más.svg'))
    btn_add_rec.setIconSize(QSize(18, 18))
    btn_add_rec.setMinimumHeight(42)
    ...
    field_row.addWidget(app.input_record_text, 2)
    field_row.addWidget(app.input_record_datetime, 1)
    field_row.addWidget(btn_add_rec)
    ih.addLayout(field_row)
    rp_layout.addWidget(input_frame)
    table_frame = QFrame()
    table_frame.setObjectName('remTableCard')
    table_frame.setStyleSheet(
        f"QFrame#remTableCard {{ background:{CLR_PANEL}; border-radius:18px; border:1px solid rgba(255,255,255,0.04); }}"
    )
    table_layout = QVBoxLayout(table_frame)
    table_layout.setContentsMargins(16, 16, 16, 16)
    table_layout.setSpacing(12)
    table_header = QHBoxLayout()
    table_title = QLabel('Tus recordatorios activos')
    table_title.setStyleSheet(f"color:{CLR_TITLE}; font:600 18px '{FONT_FAM}';")
    ...
    table_header.addWidget(app.record_count_badge)
    table_layout.addLayout(table_header)
    helper_lbl = QLabel('Organiza tus pendientes y observa el historial en una tabla limpia y clara.')
    helper_lbl.setStyleSheet(f"color:{CLR_TEXT_IDLE}; font:500 13px '{FONT_FAM}';")
    helper_lbl.setWordWrap(True)
    table_layout.addWidget(helper_lbl)
    ...
    hdr.setStyleSheet(
        f"QHeaderView::section {{ background:{CLR_HEADER_BG}; color:{CLR_HEADER_TEXT}; padding:10px; font:600 14px '{FONT_FAM}'; border:none; border-radius:12px; }}"
    )
    ...
    app.table_recordatorios.setAlternatingRowColors(True)
    app.table_recordatorios.setStyleSheet(
        app.table_recordatorios.styleSheet()
        + f" QTableWidget::item {{ padding:12px; }} QTableWidget::item:alternate {{ background:{CLR_SURFACE}; }}"
    )
    app.table_recordatorios.setColumnWidth(0, 200)
    app.table_recordatorios.setColumnWidth(1, 320)
    app.record_empty_label = QLabel('Sin recordatorios todavía. Añade el primero para comenzar.')
    app.record_empty_label.setStyleSheet(f"color:{CLR_TEXT_IDLE}; font:600 14px '{FONT_FAM}';")
    app.record_empty_label.setAlignment(Qt.AlignCenter)
    make_shadow(table_frame, 20, 4, 140)
    table_layout.addWidget(app.table_recordatorios)
    table_layout.addWidget(app.record_empty_label)
    app.record_empty_label.hide()
    rp_layout.addWidget(table_frame, 1)
    btn_del_rec = QPushButton('Eliminar seleccionado')
    btn_del_rec.setIcon(icon('Papelera.svg'))
    btn_del_rec.setIconSize(QSize(18, 18))
    btn_del_rec.setMinimumHeight(40)
    btn_del_rec.setCursor(Qt.PointingHandCursor)
    btn_del_rec.setStyleSheet(button_style(CLR_TITLE, '8px 18px'))
