...
def build_more_page(app):
    ...
    helper_lbl = QLabel('Organiza tareas en tarjetas limpias con controles de edición rápidos como en alarmas y timers.')
    ...
    rem_toolbar = QFrame()
    rem_toolbar.setObjectName('remToolbar')
    rem_toolbar.setStyleSheet(f"QFrame#remToolbar {{ background:{CLR_SURFACE}; border-radius:16px; padding:6px; }}")
    rem_tb = QHBoxLayout(rem_toolbar)
    rem_tb.setContentsMargins(6, 6, 6, 6)
    rem_tb.setSpacing(6)
    app.edit_reminder_mode_btn = QToolButton()
    app.edit_reminder_mode_btn.setCursor(Qt.PointingHandCursor)
    app.edit_reminder_mode_btn.setFixedSize(46, 38)
    app.edit_reminder_mode_btn.setStyleSheet(pill_button_style())
    edit_icon = icon('pen-to-square.svg')
    if not edit_icon.isNull():
        app.edit_reminder_mode_btn.setIcon(edit_icon)
        app.edit_reminder_mode_btn.setIconSize(QSize(20, 20))
    else:
        app.edit_reminder_mode_btn.setText('✏')
    rem_tb.addWidget(app.edit_reminder_mode_btn)
    ...
    rem_tb.addWidget(app.add_reminder_btn)
    tool_wrap = QHBoxLayout()
    tool_wrap.setContentsMargins(0, 0, 0, 0)
    tool_wrap.addStretch(1)
    tool_wrap.addWidget(rem_toolbar)
    board_layout.addLayout(tool_wrap)
    cards_scroll = QScrollArea()
    cards_scroll.setWidgetResizable(True)
    cards_scroll.setFrameShape(QFrame.NoFrame)
    cards_scroll.setVerticalScrollBar(CustomScrollBar(Qt.Vertical))
    app.reminder_cards_scroll = cards_scroll
    app.reminder_cards_container = QWidget()
    app.reminder_cards_container.setStyleSheet('background:transparent;')
    cards_grid = QGridLayout(app.reminder_cards_container)
    cards_grid.setContentsMargins(0, 0, 0, 0)
    cards_grid.setSpacing(16)
    cards_grid.setColumnStretch(0, 1)
    cards_grid.setColumnStretch(1, 1)
    app.reminder_cards_layout = cards_grid
    cards_scroll.setWidget(app.reminder_cards_container)
    board_layout.addWidget(cards_scroll, 1)
