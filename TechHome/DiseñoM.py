...
def build_more_page(app):
    ...
    cards_frame = QFrame()
    cards_frame.setStyleSheet(f'background:{CLR_PANEL}; border:2px solid {CLR_TITLE}; border-radius:12px;')
    ...
    hdr_timer = QLabel('Timers Activos')
    hdr_timer.setStyleSheet(f"color:{CLR_TEXT_IDLE}; font:600 18px '{FONT_FAM}';")
    header_layout.addWidget(hdr_timer)
    header_layout.addStretch(1)
    cards_layout.addLayout(header_layout)
    ...
    timer_scroll.setStyleSheet(f'background:transparent; border:none;')
    cards_container = QWidget()
    cards_container.setObjectName('timerCardsContainer')
    container_layout = QVBoxLayout(cards_container)
    container_layout.setContentsMargins(0, 0, 0, 0)
    container_layout.setSpacing(16)
    container_layout.addStretch(1)
    timer_scroll.setWidget(cards_container)
    cards_layout.addWidget(timer_scroll, 1)
    app.timer_cards_container = cards_container
    app.timer_cards_layout = container_layout
    empty_label = QLabel('No hay timers activos todavía')
    empty_label.setStyleSheet(f"color:{CLR_TEXT_IDLE}; font:500 14px '{FONT_FAM}';")
    empty_label.setAlignment(Qt.AlignCenter)
    empty_label.setObjectName('timerEmptyLabel')
    cards_layout.addWidget(empty_label)
    app.timer_empty_label = empty_label
    make_shadow(cards_frame, 12, 4, 120)
    ti_l.addWidget(cards_frame, 1)
    if hasattr(app, '_refresh_timer_cards'):
        try:
