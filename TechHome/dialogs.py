...
def _style_spinbox(spin: QSpinBox, large: bool = False) -> None:
    ...
    if up_path:
        up_url = up_path.replace("\\", "/")
        arrow_rules.append(f"QSpinBox::up-arrow {{ image: url(\"{up_url}\"); width:20px; height:20px; }}")
        arrow_rules.append(f"QSpinBox::up-arrow:disabled {{ image: url(\"{up_url}\"); }}")
    else:
        arrow_rules.append("QSpinBox::up-arrow { width:0; height:0; }")
    if down_path:
        down_url = down_path.replace("\\", "/")
        arrow_rules.append(f"QSpinBox::down-arrow {{ image: url(\"{down_url}\"); width:20px; height:20px; }}")
    ...
    style = (
        f"QSpinBox {{ background:{c.CLR_SURFACE}; color:{text_color}; border:2px solid {c.CLR_ITEM_ACT}; border-radius:16px; padding-right:44px; font:600 {font_sz}px '{c.FONT_FAM}'; }}"
        f"QSpinBox::up-button {{ subcontrol-origin:border; subcontrol-position:right top; width:44px; border:none; background:transparent; }}"
        f"QSpinBox::down-button {{ subcontrol-origin:border; subcontrol-position:right bottom; width:44px; border:none; background:transparent; }}"
...
class AlarmEditorDialog(BaseFormDialog):
    ...
    def __init__(self, alarm: AlarmState | None = None, parent=None):
        ...
        def make_section(name: str, radius: int = 16,
                         margins: tuple[int, int, int, int] = (16, 16, 16, 16),
                         spacing: int = 12) -> tuple[QFrame, QVBoxLayout]:
            frame = QFrame()
            frame.setObjectName(name)
            frame.setStyleSheet(
                f"QFrame#{name} {{ background:{c.CLR_SURFACE}; border:2px solid {c.CLR_ITEM_ACT}; border-radius:{radius}px; }}"
            )
        ...
        time_panel = QFrame()
        time_panel.setObjectName("alarmTimePanel")
        time_panel.setStyleSheet(
            f"QFrame#alarmTimePanel {{ background:{c.CLR_SURFACE}; border:2px solid {c.CLR_ITEM_ACT}; border-radius:24px; }}"
        )
        time_row = QHBoxLayout(time_panel)
        time_row.setContentsMargins(24, 20, 24, 20)
        ...
        colon.setStyleSheet(f"color:{c.CLR_TITLE}; font:600 34px '{c.FONT_FAM}';")
        colon.setFixedWidth(22)
        ...
        combo_style = (
            f"QComboBox {{ background:{c.CLR_SURFACE}; color:{c.CLR_TEXT_IDLE}; padding:10px 18px; font:600 15px '{c.FONT_FAM}'; border:2px solid {c.CLR_ITEM_ACT}; border-radius:14px; }}"
            f"QComboBox:focus {{ border-color:{c.CLR_TITLE}; }}"
            f"QComboBox QAbstractItemView {{ background:{c.CLR_PANEL}; color:{c.CLR_TEXT_IDLE}; selection-background-color:{c.CLR_ITEM_ACT}; border:1px solid {c.CLR_ITEM_ACT}; }}"
        )
        self.ampm_combo.setStyleSheet(combo_style + _combo_arrow_style())
        self.ampm_combo.setMinimumWidth(120)
        self.ampm_combo.setMinimumHeight(52)
        ...
        label_panel, label_layout = make_section("alarmLabelPanel", radius=18, margins=(18, 14, 18, 14))
        label_layout.setSpacing(14)
        ...
        label_layout.addWidget(alarm_label_icon)

        self.label_edit = QLineEdit()
        self.label_edit.setPlaceholderText("Nombre de la alarma")
        label_style = (
            f"QLineEdit {{ background:{c.CLR_SURFACE}; color:{c.CLR_TEXT_IDLE}; font:500 15px '{c.FONT_FAM}'; padding:12px; border:2px solid {c.CLR_ITEM_ACT}; border-radius:14px; }}"
            f"QLineEdit:focus {{ border-color:{c.CLR_TITLE}; }}"
            f"QLineEdit::placeholder {{ color:{c.CLR_PLACEHOLDER}; }}"
        )
        self.label_edit.setStyleSheet(label_style)
        self.label_edit.setMinimumHeight(50)
        label_layout.addWidget(self.label_edit, 1)
        layout.addWidget(label_panel)

        repeat_panel, repeat_layout = make_section("alarmRepeatPanel", margins=(18, 16, 18, 18))
        repeat_header = QHBoxLayout()
        repeat_header.setSpacing(8)
        repeat_icon = QLabel()
        repeat_icon.setStyleSheet("border:none;")
        loop_pix = c.pixmap("rotate-left.svg")
        if loop_pix.isNull():
            repeat_icon.setText("↺")
            repeat_icon.setStyleSheet(f"color:{c.CLR_TITLE}; font:600 16px '{c.FONT_FAM}';")
        else:
            repeat_icon.setPixmap(c.tint_pixmap(loop_pix.scaled(20, 20, Qt.KeepAspectRatio, Qt.SmoothTransformation), QColor(c.CLR_TITLE)))
        repeat_header.addWidget(repeat_icon)
        repeat_lbl = QLabel("Repetir alarma")
        repeat_lbl.setStyleSheet(f"color:{c.CLR_TEXT_IDLE}; font:600 14px '{c.FONT_FAM}';")
        repeat_header.addWidget(repeat_lbl)
        ...
        for symbol in WEEKDAY_ORDER:
            ...
            btn.setCursor(Qt.PointingHandCursor)
            btn.setStyleSheet(
                f"QToolButton {{ background:{c.CLR_SURFACE}; color:{c.CLR_TEXT_IDLE}; border-radius:18px; padding:8px 18px; font:600 12px '{c.FONT_FAM}'; border:1px solid {c.CLR_ITEM_ACT}; }}"
        ...
        repeat_layout.addLayout(day_row)
        layout.addWidget(repeat_panel)

        sound_panel, sound_layout = make_section("alarmSoundPanel", margins=(18, 16, 18, 18))
        sound_header = QHBoxLayout()
        sound_header.setSpacing(10)
        sound_icon = QLabel()
        ...
        sound_header.addWidget(sound_lbl)
        ...
        self.sound_combo.setMinimumHeight(50)
        self.sound_combo.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
        sound_layout.addWidget(self.sound_combo)
        layout.addWidget(sound_panel)

        snooze_panel, snooze_layout = make_section("alarmSnoozePanel", margins=(18, 16, 18, 18))
        snooze_row = QHBoxLayout()
        snooze_row.setSpacing(10)
        snooze_icon = QLabel()
        snooze_icon.setStyleSheet("border:none;")
        snooze_pix = c.pixmap("bell.svg")
        if snooze_pix.isNull():
            snooze_icon.setText("⏰")
            snooze_icon.setStyleSheet(f"color:{c.CLR_TITLE}; font:600 16px '{c.FONT_FAM}';")
        else:
            snooze_icon.setPixmap(c.tint_pixmap(snooze_pix.scaled(20, 20, Qt.KeepAspectRatio, Qt.SmoothTransformation), QColor(c.CLR_TITLE)))
        snooze_row.addWidget(snooze_icon)
        ...
        self.snooze_spin.setMaximumWidth(140)
        snooze_row.addWidget(self.snooze_spin)
        snooze_layout.addLayout(snooze_row)
        layout.addWidget(snooze_panel)

        layout.addStretch(1)

        title = "Editar alarma" if alarm else "Nueva alarma"
        super().__init__(title, form, "Guardar", parent=parent, size=(440, 560))
        self._deleted = False
